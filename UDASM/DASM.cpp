...
void DASM::INTEL_X86_Exec()
{
    ...
    {
        if (this->GetCntAddress() >= 0x400660)
            break;
        ...
        switch (CntByte) {
            ...
            case OPCODE_ADD_GROUP + 0x03:
                ...
                this->GetAsmStream() << this->ParseMODRM_RM(CntByte, MODRM) << ", ";
                I8 = this->ReadByte();
                this->GetAsmStream() << (GET_1ST_BIT(I8) ? "-" + this->FormatByte(~I8 + 1) : this->FormatByte(I8)) << "h";
                break;
            ...
            case OPCODE_CALL_FAR:
                this->GetAsmStream() << InstructionDef::GetOpcode()[OPCODE_CALL_FAR] << " ";
            ...
            case OPCODE_MOV_R_IMM + REG_EDI_32 + 0x08:
                ...
                break;
            case OPCODE_ROL_M_I8 + RM8_R8:
            case OPCODE_ROL_M_I8 + RM16_R16:
                this->GetAsmStream() << InstructionDef::GetOpcode()[OPCODE_ROL_M_I8] << " ";
                MODRM = this->ReadByte();
                this->GetAsmStream() << this->ParseMemoryOperand(CntByte, MODRM) << ", " << this->ParseImmediateOperand_I8();
                break;
